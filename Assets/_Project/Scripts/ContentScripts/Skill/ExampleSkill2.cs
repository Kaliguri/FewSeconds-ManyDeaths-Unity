...
public class ExampleSkill2 : SkillScript
{
    ...
    {
        List<Vector2> areaList = CombatGridMethods.CircleAOE(characterCellCoordinate, selectedCellCoordinate);
        return areaList;
    }

    public override List<Vector2> AvailableTiles(Vector2 characterCellCoordinate, int skillIndex = 0)
    {
        List<Vector2> availableTilesList = CombatGridMethods.AllDiagonalLines(characterCellCoordinate);
